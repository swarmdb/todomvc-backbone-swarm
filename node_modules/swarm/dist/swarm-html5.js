...
module.exports = function(typeId){
    ...
    var self = this;
    var spec = new Spec(typeId);
    // validate given specifier
    if (!spec.has('/')) throw new Error('Mailformed specifier')
    // define swarm set type
    Swarm.Syncable.types[spec.get('/')] || Set.extend(spec.get('/'), {});
    // create instance of swarm Set
    if (!spec.has('#') && self.id) spec = spec.add(self.id, '#')
    self._swarm = Swarm.env.localhost.get(spec.toString());
    // return `id` back, generated or existing
    self.id = self._swarm._id;
    // define the order key
    self.comparator = self.comparator || self.model.prototype.idAttribute
    // handles `add` event
    var onAdd = function(model){
        if (model._swarm) {
            // add the object if it does not exists
            if (!self._swarm.get(model._swarm.spec().filter('/#'))) {
                self._swarm.addObject(model._swarm);
            }
        } else {
            console.warn('Model is not syncable object. It will not be added to collection');
        }
    };
    ...
    var onRemove = function(model){
        if (model._swarm) {
            if (self._swarm.get(model._swarm.spec().filter('/#'))) {
                self._swarm.removeObject(model._swarm);
            }
        } else {
            console.warn('Model is not syncable object. It will not be removed from collection');
        }
    };

    // apply all changes, delivered by pipe
    var onSwarmChange = function(spec, value){
        var typeId, obj, acc = [], id = self.model.prototype.idAttribute;
        for (typeId in value) {
            obj = {}
            obj[id] = new Spec(typeId).get('#')
            if (!!value[typeId]) {
                self.add(obj, {merge: true})
                acc.push(obj);
            } else {
              var m = self.get(obj[id]);
              m && m.destroy();
            }
        }
        return acc;
    ...
    var onReset = function(){
        var acc= {}, k;
        for (k in self._swarm.objects) {
            acc[self._swarm.objects[k].spec().filter('/#').toString()] = 0
        }
        self._swarm.change(acc);
    };

    // subscriptions
    self.listenTo(self, 'add', onAdd);
    self.listenTo(self, 'remove', onRemove);
    self.listenTo(self, 'reset', onReset);
    self._swarm.on('change', onSwarmChange);

    // overwrite sync method
    self.sync = function(method, m, options){
        if (method==='read') {
            self.trigger('request');
            setTimeout(function(){
                self._swarm.on('init', function(){
                    options.success(onSwarmChange(void(8), self._swarm.objects));
                });
            },4);
        }
    };
...
},{"./Set":13,"./Spec":15}],2:[function(require,module,exports){
"use strict";
...
/**
* On the assumption of DRY principle and to avoid to create
* new data types it was decided to implement mixin, instead
...
module.exports = function(typeId){
    ...
    Swarm.Syncable.types[spec.get('/')] || Model.extend(spec.get('/'), {
        defaults:(self.defaults || {})
    });
    // create instance of swarm model
    if (!spec.has('#') && self.id) spec = spec.add(self.id, '#')
    self._swarm = Swarm.env.localhost.get(spec.toString());
    // return generated by swarm id back
    self.set(self.idAttribute, self._swarm._id);

    // Handles all changes at the backbone model and broadcast
    // it to swarm model. Except values, which is not changed
    // and `idAttribute` value. That means that you can't
    // change `id` later.
    var onChange = function(){
        var acc = {}, k;
        for (k in self.changed) {
            if (self.changed[k]!==self._swarm[k]&&k!==self.idAttribute) {
                acc[k] = self.changed[k];
            }
        }
    ...
    // applies all delivered changes to backbone model
    var onSwarmChange = function(spec, value){
        var acc = {}, k;
        for (k in value) {
          // avoid swarm specific fields
          if (Swarm.Syncable.reFieldName.test(k)) {
            acc[k] = value[k]
          }
        }
        if (!!Object.keys(acc).length) {
    ...
    self.listenTo(self, 'change', onChange);
    self._swarm.on('set', onSwarmChange);
    // apply the current state
    var state = self.toJSON(), k, acc = {};
    // avoid to apply id and overwrite defaults values
    delete state[self.idAttribute];
    for (k in state) {
      if (self.defaults[k]!==state[k]){
        acc[k] = state[k]
      }
    }
    if (!!Object.keys(acc).length) {
    ...
    self.sync = function(method, model, options) {
        ...
        switch (method) {
            ...
            case 'read':
                self._swarm.on('init', function(){
                    // go to explicit async
                    setTimeout(function(){
                        onSwarmChange(void(8), self._swarm.pojo());
                        options.success()
                        self.trigger('sync')
                    },4);
                });
...
},{"./Model":8,"./Spec":15}],3:[function(require,module,exports){
"use strict";
...
},{"./Pipe":9,"./SecondPreciseClock":12,"./Spec":15,"./Syncable":17,"./env":20}],5:[function(require,module,exports){
"use strict";
...
Swarm.BackboneModelMixin = require('./BackboneModelMixin');
Swarm.BackboneCollectionMixin = require('./BackboneCollectionMixin');
...
},{"./BackboneCollectionMixin":1,"./BackboneModelMixin":2,"./Host":4,"./LevelStorage":6,"./LongSpec":7,"./Model":8,"./Pipe":9,"./ReactMixin":11,"./Set":13,"./SharedWebStorage":14,"./Spec":15,"./Storage":16,"./Syncable":17,"./Vector":18,"./WebSocketStream":19,"./env":20}],6:[function(require,module,exports){
"use strict";
var env = require('./env');
var Spec = require('./Spec');
...
},{"./SecondPreciseClock":12,"./Spec":15,"./Storage":16,"./env":20}],7:[function(require,module,exports){
"use strict";
...
},{"./Spec":15,"./env":20}],10:[function(require,module,exports){
"use strict";
...
},{"./Spec":15,"./env":20}],12:[function(require,module,exports){
"use strict";
...
},{"./CollectionMethodsMixin":3,"./Model":8,"./ProxyListener":10,"./Spec":15,"./Syncable":17,"./env":20}],14:[function(require,module,exports){
"use strict";
var Spec = require('./Spec');
var Storage = require('./Storage');
...
},{"./Spec":15,"./env":20}],18:[function(require,module,exports){
"use strict";
...
},{"./CollectionMethodsMixin":3,"./LongSpec":7,"./ProxyListener":10,"./Spec":15,"./Syncable":17}],19:[function(require,module,exports){
"use strict";
